...
const ViewConfirmBudget = ({ navigation, route }) => {
  ...
  const submit = async () => {
    const params = {
      data: {
        ...
        boqsend_refno: route?.params?.boqsend_refno,
      },
    };
    if (message === "confirm") {
      const data = await Provider.clientBoqConfirmWorkOrderPopupApprove(params);
      ...
      const data = await Provider.clientBoqConfirmWorkOrderPopupReject(params);
      closeModal();
      if (data?.status === "Success") {
        setText(data?.message);
        setOpen(true);
      ...
      >
        <View style={[Styles.flex1, Styles.marginBottom12]}>
          ...
          <View>
            ...
            >
              <Text>Confirm to Approve & Work Order</Text>
            </Button>
            <Button
              mode="contained"
              onPress={() => {
...
export default ViewConfirmBudget;
