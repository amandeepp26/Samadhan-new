...
const CardComponent = ({
  ...
  return (
    <View
      ...
      <LabelInput label="Availability Date" value={data.availability_date} lg />
      <HDivider />
      <LabelInput
        label="Availability Employee Count"
...
const MarkAvailabilityListScreen = ({navigation}) => {
  //#region Variables
  ...
  const FetchData = from => {
    ...
    Provider.createDFCommon(
      Provider.API_URLS.branchadmin_getemployee_availability_list,
      params,
    )
      .then(response => {
        console.log('MARK resp===========:', response.data.data, "=======================");
  ...
  const EditCallback = (data, rowMap, type) => {
    rowMap[data.item.key].closeRow();
    navigation.navigate('MarkAvailabilityScreen', {
      type: type,
      fetchData: FetchData,
      data: {
  ...
  return (
    <View style={[Styles.flex1]}>
      <Header navigation={navigation} title="Employee Availability List" />
      {isLoading ? (
        <View
          style={[
            Styles.flex1,
...
export default MarkAvailabilityListScreen;
