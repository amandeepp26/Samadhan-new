...
const SingleEmployeeReport = ({ route, navigation }) => {
    //#region Variables
    ...
    const FetchDetails = () => {
        let params = {
            data: {
                ...
                month_refno: route.params?.month,
                year_refno: route.params?.year
            },
        };
        console.log('EMP params:**********', params, "*======================*");
        Provider.createDFCommon(
            Provider.API_URLS.searchresult_branchwise_attendancereport,
            params,
        )
            .then(response => {
                ...
                if (response.data && response.data.code === 200) {
                    ...
                    if (response.data.data) {
                        console.log('step 1');
                        let new_data = [];
                        response.data.data[0].attendance_data.map((data, idx) => {
                            new_data.push({
                                id: data.idx,
                                day: data?.date,
                                month: data?.month_shortname,
                                fullDate: data?.attendance_date,
                                attendanceby: data?.attendance_details,
                                advanceAmount: data?.advance_amount,
                                presentDayCount: data.present_day_count,
                            });
                        });
