...
const VerifySource = ({route, navigation}) => {
  ...
  const GetUserID = async () => {
    ...
    if (userData !== null) {
      ...
      if (route.params.type === 'edit') {
        ...
        await FetchPayToCompanyData(route.params.data.pck_trans_refno);
      } else if (
        route.params.type ===
        projectVariables.DEF_PCKDIARY_Dynamic_Expense_ClientAmountGivenToCompany_FlagText
      ) {
        await FetchPayToCompanyData(route.params.data.transactionID);
      }
    }
  };
  ...
  const FetchPayToCompanyData = transactionID => {
    let params = {
      data: {
        ...
        pck_actiontype:
          route.params.mode === 'Source'
            ? companyPaymentVerification.DEF_COMPANYSOURCE_VERIFY
            : companyPaymentVerification.DEF_COMPANYEXPENSES_VERIFY,
      },
    };
    console.log('api', Provider.API_URLS.pcktransrefnocheck);
        ...
        if (response.data && response.data.code === 200) {
          if (response.data.data) {
            ...
            route.params.data = {
              ...
              ...response.data.data[0].activitydata,
            };
  ...
  const SetEditData = data => {
    console.log(data);
    setExpenses(data?.pck_category_name);
            ...
            if (route.params.tabIndex != null && route.params.type === 'add') {
              ...
              setValue(
                response.data.data.find(
                  data => data.pck_transtype_refno == route.params.tabIndex + 1,
                )?.pck_transtype_name,
              );
            }
            if (route?.params?.type === 'edit') {
  ...
  useEffect(() => {
    if (
      route?.params?.data?.pck_custom_category_refno !== null &&
      route?.params?.data?.pck_custom_category_refno !== '' &&
      route?.params?.data?.pck_custom_category_refno !== undefined &&
      epaData.length === 0
    ) {
      let params = {
        data: {
      ...
      // console.log("epaparams", params);
      Provider.createDFPocketDairy(
        Provider.API_URLS.getexpensesto_pckaddexpensesform,
        params,
      ).then(response => {
        // console.log("eparesp", response.data);
        if (response.data && response.data.code === 200) {
          if (response.data.data) {
            // console.log(response.data.data);
            setEpaFullData(response.data.data);
            const category = response.data.data.map(data => data.category_name);
            setEpaData(category);
  ...
  useEffect(() => {
    if (
      ...
      route.params.data?.activity_entry_type !== null &&
      route.params.data?.activity_entry_type !== undefined &&
  ...
  }, [route?.params?.data?.activity_entry_type, clientTypeData]);
      ...
      .then(response => {
        if (response.data && response.data.code === 200) {
          if (response.data.data) {
            ...
            if (clientID != null) {
              console.log('client');
              let x = response.data.data.filter(el => {
                return el.myclient_refno == clientID;
              })[0];
              setClientList(x?.companyName);
            }
          }
        }
      })
        ...
        if (response.data && response.data.code === 200) {
          if (response.data.data) {
            ...
            if (editID != null) {
              setExpenses(
                response.data.data.filter(el => {
                  return el.pck_category_refno === editID;
                })[0].pck_category_name,
              );
            }
          }
  ...
  const getClientList = id => {
    ...
    if (id == undefined || id == null) {
      Provider.createDFPocketDairy(
        Provider.API_URLS.get_pckmyclientname,
        params,
      ).then(response => {
        if (response.data.data) {
          ...
          if (
            ...
            let x = response.data.data?.find(
              data => data.myclient_refno == route.params?.data?.myclient_refno,
            );
            console.log(
              'api',
  ...
  const ValidateData = async () => {
    ...
    let params = {
      ...
      Sess_branch_refno: branchID,
      Sess_designation_refno: designID,
      pck_trans_refno: data?.pck_trans_refno,
      pck_master_trans_refno: data?.pck_master_trans_refno,
      pck_entrytype_refno: data?.pck_entrytype_refno,
      pck_mode_refno: data?.pck_mode_refno,
      pck_category_refno: data?.pck_category_refno,
      pck_sub_category_refno: data?.pck_sub_category_refno,
      pck_sub_category_notes: data?.pck_sub_category_notes,
      pck_contacttype_refno: data?.pck_contacttype_refno,
      contact_name: data?.contact_name,
      contact_phoneno: data?.contact_phoneno,
      pck_mycontact_refno: data?.pck_mycontact_refno,
      pck_mybank_refno: data?.pck_mybank_refno,
      utr_no: data?.utr_no,
      cheque_no: data?.cheque_no,
      deposit_type_refno: data?.deposit_type_refno,
      cheque_date: data?.cheque_date,
      // amount: data?.amount,
      notes: data?.notes,
      reminder_date: data?.reminder_date,
      myclient_refno: data?.myclient_refno,
      cont_project_refno: data?.cont_project_refno,
      invoice_no: data?.invoice_no,
      payment_type_refno: data?.payment_type_refno,
      payment_group_refno: data?.payment_group_refno,
    ...
    console.log('params', JSON.stringify(cleanObject(params)));
    let formData = new FormData();
    formData.append('data', JSON.stringify(cleanObject(params)));
    ...
    Provider.createDFPocketDairyWithHeader(
      ...
      .then(response => {
        ...
        if (response.data && response.data?.data?.Verified == 1) {
          route.params.fetchData('update');
          navigation.goBack();
        }
      })
    ...
    <View style={[Styles.flex1]}>
      <ScrollView
        ...
        <View style={[Styles.padding16]}>
                  ...
                  {valueData.map(item => {
                    return (
                      <View
                        ...
                        <Text>{item}</Text>
                        <RadioButton disabled value={item} />
                      </View>
                    );
                  })}
                </View>
          ...
          {entryType !== 'Self' && epa && activityType !== 'Office Work' && (
            <RadioButton.Group
              onValueChange={newValue => {
              ...
              <View style={{flexDirection: 'row'}}>
                {clientTypeData.map((item, idx) => (
                  <View
                    ...
                    <RadioButton value={item} disabled={true} />
                    <Text>{item}</Text>
                  </View>
                ))}
              </View>
          ...
          {visitLocation !== '' &&
            ...
            ((entryType !== 'Self' && route.params.mode !== 'Source') ||
              route.params.mode !== 'Expenses') && (
              <>
                <TextInput
                  mode="outlined"
                ...
                <View style={{flexDirection: 'row'}}>
                  {activityStatusData.map((item, idx) => (
                    <View
                      ...
                      <RadioButton disabled={true} value={item} />
                      <Text>{item}</Text>
                    </View>
                  ))}
                </View>
          ...
          {bankStatus && (
            <>
              <View
                ...
                <Dropdown
                  label={`${
                    value === 'Source' && payMode == 'Cheque' ? 'Company' : 'My'
                  } Bank List *`}
                  data={myBankData}
            ...
            <>
              <View
                ...
                <Image
                  ...
                  resizeMethod="scale"
                />
                <Button mode="text" disabled onPress={pickDocument}>
