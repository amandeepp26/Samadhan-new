...
const RenderItems = (data, navigation, index, setIndex, setEditIndex) => {
  return (
    <View
      style={[
      ...
      >
        {data.item.action_button.includes("Edit") && (
          <Button
            ...
            onPress={() => {
              setIndex(0);
              setEditIndex(data.item.budget_refno);
            }}
            style={{
              borderColor: theme.colors.primary,
              borderWidth: 1.2,
...
const ApprovedPending = ({ index, navigation, setIndex, setEditIndex }) => {
  ...
  const fetchPending = () => {
    Provider.createDFArchitect(
      Provider.API_URLS.architect_budget_approved_pendng_list,
      ...
      <ScrollView>
        <View style={[Styles.flex1, Styles.padding16]}>
          <SwipeListView
            ...
            rightOpenValue={-160}
            renderItem={(data) =>
              RenderItems(data, navigation, index, setIndex, setEditIndex)
            }
          />
        </View>
      </ScrollView>
